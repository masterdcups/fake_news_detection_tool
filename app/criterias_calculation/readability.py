...
def calcul_tab_categorie(mediane, tab_score):
    ...
    # min = tab_min_max[0][1]
    # max = tab_min_max[0][2]
    # for i in range(1, len(tab_min_max)):
    #     if tab_min_max[i][1] < min:
    #         min = tab_min_max[i][1]
    #     if tab_min_max[i][2] > max:
    #         max = tab_min_max[i][2]
    #
    # prem_quartile = tab_score[ceil(len(tab_score) / 4.)]
    # trois_quartile = tab_score[ceil((3 * len(tab_score)) / 4.)]
    #
    # tab = [[min, prem_quartile],[prem_quartile,mediane],[mediane,trois_quartile],[trois_quartile,max]]
...
def lecture_fich():
    fichier = "././readability_scale.txt"
    tab_min_max = []
    file = open(fichier, "r").read()
    tab = file.split("]")
    for i in range(7):
